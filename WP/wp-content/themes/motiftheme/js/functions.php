...
function motiftheme_scripts() {
	...
	// Tabes js 
	wp_enqueue_script( 'jquery-min-js', get_template_directory_uri() . '/js/jquery-1.7.1.min.js', array( 'jquery' ), '',false );
	wp_enqueue_script( 'hashchange-script', get_template_directory_uri() . '/js/jquery.hashchange.min.js', array( 'jquery' ), '', true );
	wp_enqueue_script( 'easytabs-js', get_template_directory_uri() . '/js/jquery.easytabs.js', array( 'jquery' ), '', true );
...
function motif_custom_meta() {
    add_meta_box( 'motif_meta', __( 'Categories', 'motif-textdomain' ), 'motif_meta_callback', 'page','side' );
}
...
function motif_meta_callback( $post ) {
    ...
    $motif_stored_meta = get_post_meta( $post->ID );
  ?>
  
    <p>
    <span class="motif-row-title"><?php _e( 'Selecet Catagories', 'motif-textdomain' )?></span>
    <div class="motif-row-content">
    <?php  
 $categories = get_categories();
$count=1;
 foreach ($categories as $category) {  ?>
 	<label for="<?php $category->cat_name ?>">
            <input type="checkbox" name="<?php echo  'category'.$count; ?>" id="<?php echo 'category'.$count; ?>" value="yes" <?php if ( isset ( $motif_stored_meta['category'.$count] ) ) checked( $motif_stored_meta['category'.$count][0], 'yes' ); ?> />
            <?php _e( $category->cat_name, 'motif-textdomain' )?>
        </label><br>
         <?php	$count++;  }    ?>
    </div>
</p>
...
function motif_meta_save( $post_id ) {
 ...
 foreach ($categories as $category) {
	...
	if( isset( $_POST['category'.$count] ) ) {   update_post_meta( $post_id,'category'.$count, 'yes' );  } else {  update_post_meta( $post_id,'category'.$count, '' ); }
 $count++; 
   } 
